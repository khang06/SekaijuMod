...
BOOL CEventListIndexPropertyView::EndTextEditingOK () {
	...
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	if (m_lCurRow < 0 || m_lCurRow >= pEventListFrame->GetVisibleEventCount ()) {
		return FALSE;
	}
	// 現在のセルの編集テキストを取得
	CString strBuf;
	...
	case 5:
		if (MIDIEvent_IsNoteOn (pMIDIEvent) ||
			...
			if (pSekaijuApp->m_theGeneralOption.m_bChangeUpdate) {
				long lPort = MIDITrack_GetOutputPort (pMIDITrack);
				long lChannel = MIDITrack_GetOutputChannel (pMIDITrack);
				if (lChannel < 0) {
					lChannel = MIDIEvent_GetChannel (pMIDIEvent);
				}
...
CString CEventListIndexPropertyView::GetCellString (long lRow, long lColumn) {
	...
	case 5:
		...
		// テンポイベント
			...
			strText.Format (strFormat, dTempoBPM, lTempo);
		}
		// SMPTEオフセット
		...
		// その他のイベント(16進ダンプ)
			...
			memset (szText, 0, sizeof (szText));
			MIDIEvent_GetData (pMIDIEvent, byBin, sizeof (byBin));
			bin2txt (byBin, MIDIEvent_GetLen (pMIDIEvent), szText, TSIZEOF (szText) - 1);
			strText = szText;
		}
		break;
	// 値2
...
BOOL CEventListIndexPropertyView::SetCellString (long lRow, long lColumn, CString strText) {
	...
	case 5:
		...
		// テンポイベント
			...
			double dTempoBPM = _tstof (strText);
			dTempoBPM = CLIP (4.00, dTempoBPM, 60000.00);
			lValue = (long)(60000000 / dTempoBPM);
		...
		// シーケンサ固有のイベント
			...
			long lRet = txt2bin ((TCHAR*)(LPCTSTR)strText, strText.GetLength (), byData, sizeof (byData));
		...
		// システムエクスクルーシブ(通常)
			...
			if (pSekaijuApp->m_theGeneralOption.m_bChangeUpdate) {
				...
				long lChannel = MIDITrack_GetOutputChannel (pMIDITrack);
				if (lChannel < 0) {
					lChannel = MIDIEvent_GetChannel (pMIDIEvent); //TODO:最初のトラックの場合問題あり
				}
				long lTime = CLIP (0, pSekaijuDoc->m_lNewTime - 1, 0x7FFFFFFF);
				pSekaijuApp->SendAllNoteOffEx (lPort, lChannel);
				pSekaijuApp->ResetTempMIDIStatusArray ();
...
BOOL CEventListIndexPropertyView::MoveTextBox (long lRow, long lColumn) {
	...
	long lHeight = pSekaijuApp->m_theFontOption.m_lFontSize[0] + 2;
	m_theTextBox.MoveWindow 
		(rcNewCell.left, rcNewCell.top + pEventListFrame->GetRowZoom () / 2 - lHeight / 2, 
		rcNewCell.Width () - (lColumn >= 0 ? lButtonWidth : 1), lHeight);
	m_theTextBox.SetFont (&(pSekaijuApp->m_theFont[0]));
	return TRUE;
}
...
BOOL CEventListIndexPropertyView::MoveListBox (long lRow, long lColumn) {
	...
	m_theListBox.SetFont (&(pSekaijuApp->m_theFont[0]));
	return TRUE;
}
...
BOOL CEventListIndexPropertyView::ShowPopupMenu (CPoint ptMenu) {
	...
	CMenu theMenu;
	if (pSekaijuDoc->m_pTempEvent && MIDIEvent_IsSysExEvent (pSekaijuDoc->m_pTempEvent)) {
		VERIFY (theMenu.LoadMenu (IDR_POPUPMENU26));
	}
	else if (pSekaijuDoc->m_pTempEvent && MIDIEvent_IsProgramChange (pSekaijuDoc->m_pTempEvent)) {
		VERIFY (theMenu.LoadMenu (IDR_POPUPMENU25));
	}
	else {
...
BOOL CEventListIndexPropertyView::OnCreate (LPCREATESTRUCT lpcs) {
	...
	m_theTextBox.Create (WS_CHILD | ES_AUTOHSCROLL, 
		CRect (0, pEventListFrame->GetRowZoom () / 2 - 6, pEventListFrame->GetColumnWidth (0),
		13), 
		this, IDC_TEXTBOX);
	m_theTextBox.SetFont (&(pSekaijuApp->m_theFont[0]));
...
// マウス中ボタン押された時 
void CEventListIndexPropertyView::OnMButtonDown (UINT nFlags, CPoint point) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	// 録音中は何もしない
	...
	if (!m_bUniversalScroll) {
		pEventListFrame->m_bAutoPageUpdate = FALSE;
		m_bUniversalScroll = TRUE;
		m_ptMouseDownM = m_ptMouseMoveM = point;
		SetTimer (0x0901, 55, NULL);
	}
	else {
		KillTimer (0x0901);
		m_bUniversalScroll = FALSE;
...
// マウス中ボタン離されたとき
void CEventListIndexPropertyView::OnMButtonUp (UINT nFlags, CPoint point) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	CEventListFrame* pEventListFrame = (CEventListFrame*)GetParent ();
	// 録音中は何もしない
	...
	// 中ボタンキャプター中ならば
	if (GetCapture () == this && m_lTempMode >= 0x0900) {
		m_lTempMode = 0;
		ReleaseCapture ();
		::Sleep (0);
	}
...
void CEventListIndexPropertyView::OnMouseMove (UINT nFlags, CPoint point) {
	...
	CClientDC dc (this);
	OnPrepareDC (&dc, NULL);
	CSize sizWndOrg (pEventListFrame->GetColumnScrollPos (), pEventListFrame->GetRowScrollPos ());
	point += sizWndOrg;

	m_ptMouseMove = point;
	m_ptMouseMoveM = point - sizWndOrg;
}
...
void CEventListIndexPropertyView::OnTimer (UINT nIDEvent) {	
	...
	// 再生時などのカレントセル自動移動用(ページ更新機能付き)(2016/01/10ちらつき修正)
	...
	// ユニバーサルスクロール処理
	else if (nIDEvent == 0x0901) {
		long lX = pEventListFrame->GetColumnScrollPos ();
		long lY = pEventListFrame->GetRowScrollPos ();
		long lDeltaX = m_ptMouseMoveM.x - m_ptMouseDownM.x;
		long lDeltaY = m_ptMouseMoveM.y - m_ptMouseDownM.y;
		pEventListFrame->SetColumnScrollPos (lX + lDeltaX / 4);
		pEventListFrame->SetRowScrollPos (lY + lDeltaY / 4);
	}
}
