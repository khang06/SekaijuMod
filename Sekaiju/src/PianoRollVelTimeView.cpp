...
void CPianoRollVelTimeView::OnDraw (CDC* pDC) {
	...
	if (GetCapture () == this) {
		...
		else if (m_lTempMode == 0x0201) { // ライン
			if (m_lTempLineMode == 0) { // 直線
				pDC->SetROP2 (R2_NOT);
				pDC->MoveTo (m_ptMouseDown.x, m_ptMouseDown.y);
				pDC->LineTo (m_ptMouseMove.x, m_ptMouseMove.y);
				pDC->SetROP2 (R2_COPYPEN);
			}
			else if (m_lTempLineMode == 1 || m_lTempLineMode == 2) { // 曲線
				long lDeltaX = m_ptMouseMove.x - m_ptMouseDown.x;
				long lDeltaY = m_ptMouseMove.y - m_ptMouseDown.y;
				if (lDeltaX != 0) {
					double dA = (double)lDeltaY / (double)lDeltaX / (double)lDeltaX;
					POINT pt[17];
					if (m_lTempLineMode == 1) { // 始点が水平な2次曲線
						for (j = 0; j <= 16; j++) {
							pt[j].x = lDeltaX * j / 16;
							pt[j].x += m_ptMouseDown.x;
							pt[j].y = (long)(dA * lDeltaX * j * lDeltaX * j / 256);
							pt[j].y += m_ptMouseDown.y;
						}
					}
					else { //終点が水平な2次曲線
						for (j = 0; j <= 16; j++) {
							pt[j].x = lDeltaX * (-j) / 16;
							pt[j].x += m_ptMouseMove.x;
							pt[j].y = -(long)(dA * lDeltaX * j * lDeltaX * j / 256);
							pt[j].y += m_ptMouseMove.y;
						}
					}
					pDC->SetROP2 (R2_NOT);
					pDC->Polyline (pt, 17);
...
void CPianoRollVelTimeView::OnTimer (UINT_PTR nIDEvent) {
	...
	// ユニバーサルスクロール処理
		...
		long lX = pPianoRollFrame->GetTimeScrollPos ();
		long lY = pPianoRollFrame->GetVelScrollPos ();
		long lDeltaX = m_ptMouseMoveM.x - m_ptMouseDownM.x;
		long lDeltaY = m_ptMouseMoveM.y - m_ptMouseDownM.y;
		pPianoRollFrame->SetTimeScrollPos (lX + lDeltaX / 4);
		pPianoRollFrame->SetVelScrollPos (lY + lDeltaY / 4);
	}
}
...
// マウス中ボタン押された時  
void CPianoRollVelTimeView::OnMButtonDown (UINT nFlags, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// 録音中は何もしない
			...
			if (m_lTempGraphKind == 0) {
				...
				if (m_lTempGraphSnap == 1) {
					for (lTempX = lMinX; lTempX <= lMaxX; lTempX++) {
						...
						else if (m_lTempLineMode == 0) {
							lTempTempoBPM = lDownTempoBPM + (lTempTime - lDownTime) * 
								(lOldTempoBPM - lDownTempoBPM) / (lOldTime - lDownTime);
						}
						else if (m_lTempLineMode == 1 || m_lTempLineMode == 2) {
							long lDeltaTime = lOldTime - lDownTime;
							long lDeltaTempoBPM = lOldTempoBPM - lDownTempoBPM;
							double dA = (double)lDeltaTempoBPM / (double)lDeltaTime / (double)lDeltaTime;
							if (m_lTempLineMode == 1) {
								lTempTempoBPM = lDownTempoBPM + 
								(long)(dA * (lTempTime - lDownTime) * (lTempTime - lDownTime));
							}
							else {
								lTempTempoBPM = lOldTempoBPM - 
								(long)(dA * (lTempTime - lOldTime) * (lTempTime - lOldTime));
							}
						}
						lTempTempoBPM = CLIP (1, lTempTempoBPM, 60000000);
						if (lTempTempoBPM != lTempOldTempoBPM &&
							0 <= lTempTime && lTempTime <= 0x7FFFFFFF) { // 20190516 lTempTimeの条件を追加
					...
					if (lMinTime <= lTempTime && lTempTime <= lMaxTime) {
						...
						if (MIDIEvent_IsNoteOn (pTempEvent) && pTempEvent->m_pPrevCombinedEvent == NULL) {
							...
							if (lOldTime == lDownTime) {
								...
								long lDeltaTime = lOldTime - lDownTime;
								long lDeltaVel = lOldVel - lDownVel;
								double dA = (double)lDeltaVel / (double)lDeltaTime / (double)lDeltaTime;
								if (m_lTempLineMode == 1) {
									lTempVel = lDownVel + 
									(long)(dA * (lTempTime - lDownTime) * (lTempTime - lDownTime));
								}
				...
				if (m_lTempGraphSnap == 1) {
					for (lTempX = lMinX; lTempX <= lMaxX; lTempX++) {
						...
						if (lOldTime == lDownTime) {
							...
							long lDeltaTime = lOldTime - lDownTime;
							long lDeltaValue = lOldValue - lDownValue;
							double dA = (double)lDeltaValue / (double)lDeltaTime / (double)lDeltaTime;
							if (m_lTempLineMode == 1) {
								lTempValue = lDownValue + 
								(long)(dA * (lTempTime - lDownTime) * (lTempTime - lDownTime));
							}
							else {
								lTempValue = lOldValue - 
				...
				if (m_lTempGraphSnap == 1) {
					for (lTempX = lMinX; lTempX <= lMaxX; lTempX++) {
						...
						if (lOldTime == lDownTime) {
							...
							long lDeltaTime = lOldTime - lDownTime;
							long lDeltaPitchBend = lOldPitchBend - lDownPitchBend;
							double dA = (double)lDeltaPitchBend / (double)lDeltaTime / (double)lDeltaTime;
							if (m_lTempLineMode == 1) {
								lTempPitchBend = lDownPitchBend + 
								(long)(dA * (lTempTime - lDownTime) * (lTempTime - lDownTime));
							}
							else {
								lTempPitchBend = lOldPitchBend - 
					...
					for (j = jMin; j <= jMax + 1; j++) {
						...
						if (lMinTime <= lTempTime && lTempTime <= lMaxTime) {
							...
							lTempPitchBend = CLIP (0, lTempPitchBend, 16483);
							if (lTempPitchBend != lTempOldPitchBend &&
								0 <= lTempTime && lTempTime <= 0x7FFFFFFF) { // 20190516 lTempTimeの条件を追加								VERIFY (pTempEvent = MIDIEvent_CreatePitchBend
								VERIFY (pTempEvent = MIDIEvent_CreatePitchBend
									(lTempTime, m_lTempChannel, lTempPitchBend));
								VERIFY (MIDITrack_InsertEvent (pTempTrack, pTempEvent));
								pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pTempEvent);
...
// マウス中ボタン離されたとき
void CPianoRollVelTimeView::OnMButtonUp (UINT nFlags, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
}
