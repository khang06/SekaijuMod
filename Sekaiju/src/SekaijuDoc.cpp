...
BEGIN_MESSAGE_MAP (CSekaijuDoc, CDocument)
	...
	ON_COMMAND (ID_EDIT_TEMPO, OnEditTempo)
	ON_UPDATE_COMMAND_UI (ID_EDIT_TEMPO, OnUpdateEditTempoUI)
	...
	ON_COMMAND (ID_EDIT_STROKE, OnEditStroke)
	ON_UPDATE_COMMAND_UI (ID_EDIT_STROKE, OnUpdateEditStrokeUI)

	ON_COMMAND (ID_EDIT_INSERTMEASURE, OnEditInsertMeasure)
	ON_UPDATE_COMMAND_UI (ID_EDIT_INSERTMEASURE, OnUpdateEditInsertMeasureUI)
	ON_COMMAND (ID_EDIT_REMOVEMEASURE, OnEditRemoveMeasure)
	ON_UPDATE_COMMAND_UI (ID_EDIT_REMOVEMEASURE, OnUpdateEditRemoveMeasureUI)

	ON_COMMAND (ID_EDIT_SAL, OnEditSAL)
	ON_UPDATE_COMMAND_UI (ID_EDIT_SAL, OnUpdateEditSALUI)
	ON_COMMAND (ID_EDIT_TERMINATESAL, OnEditTerminateSAL)
	ON_UPDATE_COMMAND_UI (ID_EDIT_TERMINATESAL, OnUpdateEditTerminateSALUI)
	...
	ON_COMMAND (ID_VIEW_MUSICALSCORE, OnViewMusicalScore)

	ON_COMMAND (ID_POPUP_SHOWTRACKLIST, OnPopupShowTrackList)
	ON_UPDATE_COMMAND_UI (ID_POPUP_SHOWTRACKLIST, OnUpdatePopupShowTrackListUI)
	ON_COMMAND (ID_POPUP_SHOWPIANOROLL, OnPopupShowPianoRoll)
	ON_UPDATE_COMMAND_UI (ID_POPUP_SHOWPIANOROLL, OnUpdatePopupShowPianoRollUI)
	ON_COMMAND (ID_POPUP_SHOWEVENTLIST, OnPopupShowEventList)
	ON_UPDATE_COMMAND_UI (ID_POPUP_SHOWEVENTLIST, OnUpdatePopupShowEventListUI)
	ON_COMMAND (ID_POPUP_SHOWMUSICALSCORE, OnPopupShowMusicalScore)
	ON_UPDATE_COMMAND_UI (ID_POPUP_SHOWMUSICALSCORE, OnUpdatePopupShowMusicalScoreUI)
	...
	ON_COMMAND (ID_POPUP_SELECTVOICE, OnPopupSelectVoice)
	ON_UPDATE_COMMAND_UI (ID_POPUP_SELECTVOICE, OnUpdatePopupSelectVoiceUI)
	ON_COMMAND (ID_POPUP_LOADSYSEXDATA, OnPopupLoadSysExData)
	ON_UPDATE_COMMAND_UI (ID_POPUP_LOADSYSEXDATA, OnUpdatePopupLoadSysExDataUI)
	ON_COMMAND (ID_POPUP_SAVESYSEXDATA, OnPopupSaveSysExData)
	ON_UPDATE_COMMAND_UI (ID_POPUP_SAVESYSEXDATA, OnUpdatePopupSaveSysExDataUI)


	// 以下コマンド用
	ON_COMMAND (ID_COMMAND_STOPPLAYING, OnCommandStopPlaying)
	ON_COMMAND (ID_COMMAND_STOPRECORDING, OnCommandStopRecording)
...
BOOL CSekaijuDoc::OnOpenDocument (LPCTSTR lpszPathName) {
	...
	// Cakewalkシーケンスファイル(*.wrk)の場合
	else if (strExt.CompareNoCase (_T(".wrk")) == 0) {
		m_pMIDIData = MIDIData_LoadFromWRK (lpszPathName);
		if (m_pMIDIData == NULL) {
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
			CString strFormat;
	...
	// マビノギMMLファイル(*.mmml)の場合
	else if (strExt5.CompareNoCase (_T(".mmml")) == 0) {
		m_pMIDIData = MIDIData_LoadFromMabiMML (lpszPathName);
		if (m_pMIDIData == NULL) {
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
			CString strFormat;
	...
	// 異常なメタイベントのチェック(20190915追加)
	long lNumAbnormalMetaEvent = 0;
	CPtrArray theAbnormalMetaEventArray;
	forEachTrack (m_pMIDIData, pMIDITrack) {
		forEachEvent (pMIDITrack, pMIDIEvent) {
			if (MIDIEvent_IsPortPrefix (pMIDIEvent) && MIDIEvent_GetLen (pMIDIEvent) < 1) {
				lNumAbnormalMetaEvent++;
				theAbnormalMetaEventArray.Add (pMIDIEvent);
			}
			else if (MIDIEvent_IsChannelPrefix (pMIDIEvent) && MIDIEvent_GetLen (pMIDIEvent) < 1) {
				lNumAbnormalMetaEvent++;
				theAbnormalMetaEventArray.Add (pMIDIEvent);
			}
			else if (MIDIEvent_IsSMPTEOffset (pMIDIEvent) && MIDIEvent_GetLen (pMIDIEvent) < 5) {
				lNumAbnormalMetaEvent++;
				theAbnormalMetaEventArray.Add (pMIDIEvent);
			}
			else if (MIDIEvent_IsTempo (pMIDIEvent) && MIDIEvent_GetLen (pMIDIEvent) < 3) {
				lNumAbnormalMetaEvent++;
				theAbnormalMetaEventArray.Add (pMIDIEvent);
			}
			else if (MIDIEvent_IsTimeSignature (pMIDIEvent) && MIDIEvent_GetLen (pMIDIEvent) < 4) {
				lNumAbnormalMetaEvent++;
				theAbnormalMetaEventArray.Add (pMIDIEvent);
			}
			else if (MIDIEvent_IsKeySignature (pMIDIEvent) && MIDIEvent_GetLen (pMIDIEvent) < 2) {
				lNumAbnormalMetaEvent++;
				theAbnormalMetaEventArray.Add (pMIDIEvent);
			}
		}
	}
	if (lNumAbnormalMetaEvent > 0) {
		m_theCriticalSection.Unlock ();
		EndWaitCursor ();
		CString strFormat;
		VERIFY (strFormat.LoadString (IDS_D_METAEVENTS_ARE_ABNORMAL_ARE_YOU_SURE_TO_DELETE_THEM));
		CString strMsg;
		strMsg.Format (strFormat, lNumAbnormalMetaEvent);
		long lRet = AfxMessageBox (strMsg, MB_ICONQUESTION | MB_YESNO);
		BeginWaitCursor ();
		m_theCriticalSection.Lock ();
		// 異常なメタイベントを削除
		if (lRet == IDYES) {
			for (int i = 0; i < lNumAbnormalMetaEvent; i++) {
				pMIDIEvent = (MIDIEvent*)(theAbnormalMetaEventArray.GetAt (i));
...
BOOL CSekaijuDoc::OnSaveDocument (LPCTSTR lpszPathName) {
	...
	m_theCriticalSection.Unlock ();
	// SAL実行中の場合はSAL強制停止
	if (pSekaijuApp->m_bSALRunning && pSekaijuApp->m_pSAL != NULL) {
		SAL_Terminate pfnSAL_Terminate = (SAL_Terminate)pSekaijuApp->m_pfnSAL_Terminate;
		int nResult = pfnSAL_Terminate ((pSekaijuApp->m_pSAL), 0);
		::Sleep (0);
	}
	...
	// スタンダードMIDIファイル(*.mid)の場合
		...
		// MIDIトラックのTime+適用チェック(Y/N)
		long lCountTimePlus = 0;
		forEachTrack (m_pMIDIData, pMIDITrack) {
			long lTimePlus = MIDITrack_GetTimePlus (pMIDITrack);
			if (lTimePlus != 0) {
				lCountTimePlus++;
			}
		}
		if (lCountTimePlus > 0) {
			VERIFY (strMsg1.LoadString (IDS_D_MIDITRACKS_TIMEPLUS_IS_USED_N_APPLY_TO_EACH_EVENT));
			strMsg2.Format (strMsg1, lCountTimePlus);
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
			lRet = AfxMessageBox (strMsg2, MB_ICONINFORMATION | MB_YESNO);
			BeginWaitCursor ();
				...
				forEachTrack (m_pMIDIData, pMIDITrack) {
					...
					if (lTimePlus != 0) {
						...
						if (pLastEvent != NULL && MIDIEvent_IsEndofTrack (pLastEvent)) {
							lEOTTime = MIDIEvent_GetTime (pLastEvent);
						}
						forEachEvent (pMIDITrack, pMIDIEvent) {
							// イベントで前方向に非結合イベントならば
							if (pMIDIEvent->m_pPrevCombinedEvent == NULL) {
								long lTime = 0;
								if (pMIDIEvent == pLastEvent && MIDIEvent_IsEndofTrack (pMIDIEvent)) {
									lTime = lEOTTime;
								}
								else {
						...
						// トラックのTime+を0に戻す。
						VERIFY (pCurHistoryUnit->AddHistoryRecord 
							(HISTORYRECORD_REMOVETRACK, pMIDITrack));
						MIDITrack* pCloneTrack = ReplaceMIDITrack (pMIDITrack);
						MIDITrack_SetTimePlus (pCloneTrack, 0);
						VERIFY (pCurHistoryUnit->AddHistoryRecord 
							(HISTORYRECORD_INSERTTRACK, pCloneTrack));
						pMIDITrack = pCloneTrack;
					}
				}
			}
		}
		// MIDIトラックのKeyPlus+適用チェック(Y/N)
		long lCountKeyPlus = 0;
		forEachTrack (m_pMIDIData, pMIDITrack) {
			long lKeyPlus = MIDITrack_GetKeyPlus (pMIDITrack);
			if (lKeyPlus != 0) {
				lCountKeyPlus++;
			}
		}
		if (lCountKeyPlus > 0) {
			VERIFY (strMsg1.LoadString (IDS_D_MIDITRACKS_KEYPLUS_IS_USED_N_APPLY_TO_EACH_EVENT));
			strMsg2.Format (strMsg1, lCountKeyPlus);
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
			lRet = AfxMessageBox (strMsg2, MB_ICONINFORMATION | MB_YESNO);
			BeginWaitCursor ();
					...
					if (lKeyPlus != 0) {
						forEachEvent (pMIDITrack, pMIDIEvent) {
							...
							if (MIDIEvent_IsNote (pMIDIEvent) && 
								...
								MIDIEvent_SetKey (pCloneEvent, lKey);
								VERIFY (pCurHistoryUnit->AddHistoryRecord 
									(HISTORYRECORD_INSERTEVENT, pCloneEvent));
								pMIDIEvent = pCloneEvent;
							}
						...
						VERIFY (pCurHistoryUnit->AddHistoryRecord 
							(HISTORYRECORD_REMOVETRACK, pMIDITrack));
						MIDITrack* pCloneTrack = ReplaceMIDITrack (pMIDITrack);
						MIDITrack_SetKeyPlus (pCloneTrack, 0);
						VERIFY (pCurHistoryUnit->AddHistoryRecord 
							(HISTORYRECORD_INSERTTRACK, pCloneTrack));
						pMIDITrack = pCloneTrack;
					}
		...
		// MIDIトラックのVel+適用チェック(Y/N)
		long lCountVelPlus = 0;
		forEachTrack (m_pMIDIData, pMIDITrack) {
			long lVelPlus = MIDITrack_GetVelocityPlus (pMIDITrack);
			if (lVelPlus != 0) {
				lCountVelPlus++;
			}
		}
		if (lCountVelPlus > 0) {
			VERIFY (strMsg1.LoadString (IDS_D_MIDITRACKS_VELPLUS_IS_USED_N_APPLY_TO_EACH_EVENT));
			strMsg2.Format (strMsg1, lCountVelPlus);
			m_theCriticalSection.Unlock ();
			EndWaitCursor ();
			lRet = AfxMessageBox (strMsg2, MB_ICONINFORMATION | MB_YESNO);
			BeginWaitCursor ();
			...
			if (lRet == IDYES) {
				...
				VERIFY (pCurHistoryUnit = GetCurHistoryUnit ());
				forEachTrack (m_pMIDIData, pMIDITrack) {
					long lVelocityPlus = MIDITrack_GetVelocityPlus (pMIDITrack);
					if (lVelocityPlus != 0) {
						forEachEvent (pMIDITrack, pMIDIEvent) {
							// ノートイベントで前方向に非結合イベントならば
							if (MIDIEvent_IsNote (pMIDIEvent) && 
								...
								pCloneEvent = ReplaceMIDIEvent (pMIDIEvent);
								lVel += lVelocityPlus;
								lVel = (CLIP (1, lVel, 127));
								MIDIEvent_SetVelocity (pCloneEvent, lVel);
								VERIFY (pCurHistoryUnit->AddHistoryRecord 
									(HISTORYRECORD_INSERTEVENT, pCloneEvent));
								pMIDIEvent = pCloneEvent;
							}
						...
						VERIFY (pCurHistoryUnit->AddHistoryRecord 
							(HISTORYRECORD_REMOVETRACK, pMIDITrack));
						MIDITrack* pCloneTrack = ReplaceMIDITrack (pMIDITrack);
						MIDITrack_SetVelocityPlus (pCloneTrack, 0);
						VERIFY (pCurHistoryUnit->AddHistoryRecord 
							(HISTORYRECORD_INSERTTRACK, pCloneTrack));
						pMIDITrack = pCloneTrack;
					}
...
void CSekaijuDoc::TimeMIDIStatus (long lTargetTime, long lKeyShift, MIDIStatus* pMIDIStatus[]) {
	MIDITrack* pMIDITrack;
	MIDIEvent* pMIDIEvent;
	...
	forEachTrack (m_pMIDIData, pMIDITrack) {
		if (MIDITrack_GetOutputOn (pMIDITrack)) {
			...
			long lOutputPort = MIDITrack_GetOutputPort (pMIDITrack);
			long lOutputChannel = MIDITrack_GetOutputChannel (pMIDITrack);
			long lKeyShift2 = (MIDITrack_GetViewMode (pMIDITrack) == 0) ? lKeyShift : 0;
			if (lOutputOn) {
				forEachEvent (pMIDITrack, pMIDIEvent) {
					long lTime = MIDIEvent_GetTime (pMIDIEvent);
					lTime = CLIP (0, lTime + MIDITrack_GetTimePlus (pMIDITrack), 0x7FFFFFFF);
					...
					if (MIDIEvent_IsMIDIEvent (pMIDIEvent)) {
						...
						// キー+とベロシティ+の反映
						if (MIDIEvent_IsNoteOn (pMIDIEvent)) {
							byMsg[1] = (BYTE)CLIP (0, byMsg[1] + MIDITrack_GetKeyPlus (pMIDITrack) + lKeyShift2, 127);
...
MIDIEvent* CSekaijuDoc::ReplaceMIDIEvent (MIDIEvent* pMIDIEvent) {
	...
	while (pTargetEvent != NULL && pInsertEvent != NULL) {
		...
		ASSERT (pMIDITrack != NULL && pTargetEvent != NULL);
		VERIFY (MIDITrack_RemoveSingleEvent (pMIDITrack, pTargetEvent));
		ASSERT (pMIDITrack != NULL && pInsertEvent != NULL);
...
void CSekaijuDoc::OnFileProperty () {
	...
	TCHAR szBuf1[MIDIEVENT_MAXLEN];
	TCHAR szBuf2[MIDIEVENT_MAXLEN];
...
void CSekaijuDoc::OnEditUndo () {
	...
	else { // 2018/02/11 テンポの変更に対する演奏テンポと位置の補正対応
		if (pSekaijuApp->m_bPlaying) {
			MIDIClock_Stop (m_pMIDIClock);
		}
		long lCurTime = MIDIClock_GetTickCount (m_pMIDIClock);
		long lMillisec = MIDIData_TimeToMillisec (m_pMIDIData, lCurTime);
		long lTempo = MIDIEVENT_DEFTEMPO;
		VERIFY (MIDIData_FindTempo (m_pMIDIData, lCurTime, &lTempo));
		MIDIClock_SetMillisec (m_pMIDIClock, lMillisec);
		MIDIClock_SetTempo (m_pMIDIClock, lTempo);
		if (pSekaijuApp->m_bPlaying) {
			MIDIClock_Start (m_pMIDIClock);
		}
	...
	if (pSekaijuApp->m_bPlaying) {
		...
		this->TimeMIDIStatus 
			(lCurTime, pSekaijuApp->m_lKeyShift, pSekaijuApp->m_pTempMIDIStatus);
...
void CSekaijuDoc::OnEditTime () {
	...
	if (lTimeMode == 0) {
		...
		if (theDlg.DoModal () == IDOK) {
			...
			for (long j = 0; j < lTempSelectedEventArrayCount; j++) {
				pMIDIEvent = (MIDIEvent*)(theTempSelectedEventArray.GetAt (j));
				pMIDIEvent->m_lUser1 = (long)(MIDIEvent_GetParent (pMIDIEvent));
				VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pMIDIEvent));
			...
			for (long j = 0; j < lTempSelectedEventArrayCount; j++) {
				...
				if (theDlg.m_nUnit == 0) {
					...
					// EOTの場合は最後のイベントより前方に来ないようにする
					if (MIDIEvent_IsEndofTrack (pCloneEvent)) {
						MIDIEvent* pLastEvent = MIDITrack_GetLastEvent ((MIDITrack*)(pCloneEvent->m_lUser1));
						lTime = CLIP (MIDIEvent_GetTime (pLastEvent), lTime, 0x7FFFFFFF);
					}
				}
				lTime = CLIP (0, lTime, 0x7FFFFFFF);
				VERIFY (MIDIEvent_SetTime (pCloneEvent, lTime));
				VERIFY (MIDITrack_InsertEvent ((MIDITrack*)(pCloneEvent->m_lUser1), pCloneEvent));
				VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
			}
			theTempSelectedEventArray.RemoveAll ();
			// EOTの履歴保持(EOTが選択されていない場合のみ)(20081102修正)
	...
	// SMPTEベース
		...
		if (theDlg.DoModal () == IDOK) {
			...
			// 一時配列のイベント1個につき1回の移動をかける。
			long lTempSelectedEventArrayCount = theTempSelectedEventArray.GetSize ();
			// (SetTimeは同時刻イベントの最後に挿入するため、昇順に)(20170508修正)
			for (long j = 0; j < lTempSelectedEventArrayCount; j++) {
				pMIDIEvent = (MIDIEvent*)(theTempSelectedEventArray.GetAt (j));
				VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pMIDIEvent));
...
void CSekaijuDoc::OnEditTempo () {
	...
	CEditTempoDlg theDlg;
	theDlg.m_strAmount = pSekaijuApp->m_theEditTempoDlgStatus.m_strAmount;
	theDlg.m_nUnit = CLIP (0, pSekaijuApp->m_theEditTempoDlgStatus.m_nUnit, 3);
	if (theDlg.DoModal () == IDOK) {
		BeginWaitCursor ();
		m_theCriticalSection.Lock ();
		MIDITrack* pMIDITrack = NULL;
		...
		CHistoryUnit* pCurHistoryUnit = NULL;
		CString strHistoryName;
		VERIFY (strHistoryName.LoadString (IDS_EDIT_MODIFY_TEMPO));
		VERIFY (this->AddHistoryUnit (strHistoryName));
		VERIFY (pCurHistoryUnit = this->GetCurHistoryUnit ());
		// 値の変更
		forEachTrack (m_pMIDIData, pMIDITrack) {
			forEachEvent (pMIDITrack, pMIDIEvent) {
				...
				if (this->IsEventSelected (pMIDIEvent)) {
					...
					if (MIDIEvent_IsTempo (pMIDIEvent)) {
						...
						double dAmount = _tstof (theDlg.m_strAmount);
						if (theDlg.m_nUnit == 0) {
							dAmount = CLIP (1, dAmount, 1000);
							dTempoBPM = dAmount;
						}
						else if (theDlg.m_nUnit == 1) {
							dAmount = CLIP (-1000, dAmount, 1000);
							dTempoBPM += dAmount;
						}
						else if (theDlg.m_nUnit == 2) {
							dAmount = CLIP (0.001, dAmount, 1000);
							dTempoBPM = dTempoBPM * dAmount / 100;
						}
						else if (theDlg.m_nUnit == 3) {
							dAmount = CLIP (-1000, dAmount, 1000);
							long lAmount100 = (long)(dAmount * 100);
							long lDelta100 = rand () % (abs (lAmount100 * 2) + 1);
							double dDelta = (double)(lDelta100 - abs (lAmount100)) / (double)100;
							dTempoBPM += dDelta;
						}
						dTempoBPM = CLIP (4.00, dTempoBPM, 60000.00);
						lTempo = (long)(60000000 / dTempoBPM);
						VERIFY (MIDIEvent_SetTempo (pCloneEvent, lTempo));
						VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
						pMIDIEvent = pCloneEvent;
					}
				}
		...
		EndWaitCursor ();
		::Sleep (0);
		pSekaijuApp->m_theEditTempoDlgStatus.m_strAmount = theDlg.m_strAmount;
		pSekaijuApp->m_theEditTempoDlgStatus.m_nUnit = theDlg.m_nUnit;
	}
}

// 『編集(E)』-『テンポの変更...』
void CSekaijuDoc::OnUpdateEditTempoUI (CCmdUI* pCmdUI) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bSALRunning ||
		pSekaijuApp->m_bInplaceEditing ||
...
void CSekaijuDoc::OnEditStroke () {
	...
	CEditStrokeDlg theDlg;
	theDlg.m_nDeltaTime = pSekaijuApp->m_theEditStrokeDlgStatus.m_nDeltaTime;
	theDlg.m_nMode = CLIP (0, pSekaijuApp->m_theEditStrokeDlgStatus.m_nMode, 3);

	// ダイアログ表示
	if (theDlg.DoModal () == IDOK) {

		pSekaijuApp->m_theEditStrokeDlgStatus.m_nDeltaTime = theDlg.m_nDeltaTime;
		pSekaijuApp->m_theEditStrokeDlgStatus.m_nMode = theDlg.m_nMode;
		...
		CHistoryUnit* pCurHistoryUnit = NULL;
		CString strHistoryName;
		VERIFY (strHistoryName.LoadString (IDS_EDIT_STROKE));
		VERIFY (this->AddHistoryUnit (strHistoryName));
		VERIFY (pCurHistoryUnit = this->GetCurHistoryUnit ());
		...
		// 選択されているノートオンイベントを一時配列に保持するとともに
		// 和音のグループ番号を付与
		long lTrackIndex = 0;
		long lGroupIndex = 0;
		CPtrArray theTempEventArray;
		CDWordArray theTempTrackIndexArray;
		CDWordArray theTempGroupIndexArray;
		forEachTrack (m_pMIDIData, pMIDITrack) {
			long lOldTime = -1;
			forEachEvent (pMIDITrack, pMIDIEvent) {
				if (this->IsEventSelected (pMIDIEvent)) {
					if (MIDIEvent_IsNoteOn (pMIDIEvent) && MIDIEvent_IsNote (pMIDIEvent)) {
						...
						theTempTrackIndexArray.Add (lTrackIndex);
						theTempGroupIndexArray.Add (lGroupIndex);
						theTempEventArray.Add (pMIDIEvent);
						lOldTime = lTime;
					}
				}
			}
		...
		theTempTrackIndexArray.Add (lTrackIndex);
		theTempGroupIndexArray.Add (lGroupIndex);
		theTempEventArray.Add (NULL);
		
		// 各和音グループごとにノート音階順に並べ替え(バブルソート)
		long j = 0;
		long jMax = theTempEventArray.GetSize ();
		long jSortMin = 0;
		long jSortMax = 0;
		long lOldTrackIndex = 0;
		long lOldGroupIndex = 0;
		for (j = 0; j < jMax; j++) {
			lTrackIndex = theTempTrackIndexArray.GetAt (j);
			lGroupIndex = theTempGroupIndexArray.GetAt (j);
			if (lTrackIndex > lOldTrackIndex || lGroupIndex > lOldGroupIndex) {
				jSortMax = j;
				// バブルソート(昇順)
				if (theDlg.m_nMode == 0 || 
					(theDlg.m_nMode == 2 && (lOldGroupIndex % 2) == 1) ||
					(theDlg.m_nMode == 3 && (lOldGroupIndex % 2) == 0)) {
					long jj1, jj2;
					for (jj1 = jSortMin; jj1 < jSortMax - 1; jj1++) {
						for (jj2 = jSortMin; jj2 < jSortMax - 1; jj2++) {
							long lKey1 = MIDIEvent_GetKey ((MIDIEvent*)(theTempEventArray.GetAt (jj2)));
							long lKey2 = MIDIEvent_GetKey ((MIDIEvent*)(theTempEventArray.GetAt (jj2 + 1)));
							if (lKey1 > lKey2) {
								void* pTempEvent = theTempEventArray.GetAt (jj2);
								theTempEventArray.SetAt (jj2, theTempEventArray.GetAt (jj2 + 1));
								theTempEventArray.SetAt (jj2 + 1, pTempEvent);
							}
						}
					}
				}
			...
			lOldGroupIndex = lGroupIndex;
			lOldTrackIndex = lTrackIndex;
		}
		...
		for (j = 0; j < jMax - 1; j++) {
			lTrackIndex = theTempTrackIndexArray.GetAt (j);
			lGroupIndex = theTempGroupIndexArray.GetAt (j);
			pMIDIEvent = (MIDIEvent*)(theTempEventArray.GetAt (j));
			if (lTrackIndex > lOldTrackIndex || lGroupIndex > lOldGroupIndex) {
				lOverlap = 0;
			}
			long lTime = MIDIEvent_GetTime (pMIDIEvent);
			long lNewTime = lTime + theDlg.m_nDeltaTime * lOverlap;
			lNewTime = CLIP (0, lNewTime, 0x7FFF0000);
			long lDuration = MIDIEvent_GetDuration (pMIDIEvent);
			long lNewDuration = lDuration - (lNewTime - lTime);
			lNewDuration = CLIP (1, lNewDuration, 0x0000FFFF);
			if (lTime != lNewTime) {
				VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pMIDIEvent));
				VERIFY (pCloneEvent = ReplaceMIDIEvent (pMIDIEvent));
				MIDIEvent_SetTime (pCloneEvent, lNewTime);
		...
		// 一時配列の解放
		theTempTrackIndexArray.RemoveAll ();
		theTempGroupIndexArray.RemoveAll ();
		theTempEventArray.RemoveAll ();
...
// 『編集(E)』-『ストローク...』
void CSekaijuDoc::OnUpdateEditStrokeUI (CCmdUI* pCmdUI) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bSALRunning ||
		pSekaijuApp->m_bInplaceEditing ||
...
void CSekaijuDoc::OnEditInsertMeasure () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		for (long j = 0; j < lTempFollowingEventArrayCount; j++) {
			...
			VERIFY (pCloneEvent = MIDIEvent_CreateClone (pMIDIEvent));
			VERIFY (MIDITrack_RemoveEvent (MIDIEvent_GetParent (pMIDIEvent), pMIDIEvent));
			theTempFollowingEventArray.SetAt (j, pCloneEvent);
		}
		for (long j = 0; j < lTempFollowingEventArrayCount; j++) {
			pCloneEvent = (MIDIEvent*)(theTempFollowingEventArray.GetAt (j));
			long lTime = MIDIEvent_GetTime (pCloneEvent) + lSlideTime;
			lTime = CLIP (0, lTime, 0x7FFFFFFF);
			VERIFY (MIDIEvent_SetTime (pCloneEvent, lTime));
			VERIFY (MIDITrack_InsertEvent ((MIDITrack*)(pCloneEvent->m_lUser1), pCloneEvent));
			VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
...
void CSekaijuDoc::OnEditRemoveMeasure () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		for (long j = 0; j < lTempDuringEventArrayCount; j++) {
			...
			if (MIDIEvent_IsEndofTrack (pMIDIEvent)) {
				VERIFY (pCloneEvent = ReplaceMIDIEvent (pMIDIEvent));
				long lTime = CLIP (0, lTime2, 0x7FFFFFFF);
				MIDIEvent_SetTime (pCloneEvent, lTime);
				VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
				pMIDIEvent = pCloneEvent;
			}
...
void CSekaijuDoc::OnEditSAL () {
	...
	// SAL(d).dllの読み込み(このコマンド使用の初回のみ)
	if (pSekaijuApp->m_hSALDLL == NULL) {
		BOOL bRet = pSekaijuApp->LoadSALDLL ();
		if (bRet == FALSE) {
			pSekaijuApp->FreeSALDLL ();
			pSekaijuApp->m_hSALDLL = NULL;
			return;
		}
	}

	ASSERT (pSekaijuApp->m_pSAL != NULL);

	// ファイルの選択
	TCHAR szFileName[1024];
	SAL_SelectFile pfnSAL_SelectFile = (SAL_SelectFile)pSekaijuApp->m_pfnSAL_SelectFile;
	int nRet1 = pfnSAL_SelectFile (pSekaijuApp->m_pSAL, szFileName, 1024);
	if (nRet1 == 0) {
		return;
	}

	// ファイルからSALスクリプトの読み込み
	SAL_LoadFile pfnSAL_LoadFile = (SAL_LoadFile)pSekaijuApp->m_pfnSAL_LoadFile;
	int nRet2 = pfnSAL_LoadFile (pSekaijuApp->m_pSAL, szFileName, 0);
	if (nRet2 == 0) {
		return;
	}
	...
	CRuntimeClass* prt = pChildFrame->GetRuntimeClass ();
	if (strcmp (prt->m_lpszClassName, "CTrackListFrame") != 0 &&
		strcmp (prt->m_lpszClassName, "CPianoRollFrame") != 0 &&
		strcmp (prt->m_lpszClassName, "CEventListFrame") != 0 &&
		strcmp (prt->m_lpszClassName, "CMusicalScroreFrame") != 0) {
		return;
	}
	m_nSALCurTrack = pChildFrame->GetCurTrackIndex ();


	// SAL用スレッドの開始
	pSekaijuApp->m_bSALRunning = TRUE;
	pSekaijuApp->m_pSALThread = AfxBeginThread (pSekaijuApp->SALThread, this);
	if (pSekaijuApp->m_pSALThread == NULL) {
		pSekaijuApp->m_bSALRunning = FALSE;
		_RPTF1 (_CRT_WARN, "SALThread Create Failed, Address=0x%p.\n", pSekaijuApp->m_pSALThread);
		CString strMsg;
		strMsg.LoadString (IDS_SALTHREAD_CREATE_ERROR);
		AfxMessageBox (strMsg, MB_ICONSTOP);
	}
	else {
		_RPTF1 (_CRT_WARN, "SALThread Create Successful, Address=0x%p.\n", pSekaijuApp->m_pSALThread);
	}
}

// 『編集(E)』-『SAL...』(20170728追加)
void CSekaijuDoc::OnUpdateEditSALUI (CCmdUI* pCmdUI) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bSALRunning ||
		pSekaijuApp->m_bInplaceEditing ||
...
// 『編集(E)』-『SALの強制終了』非表示, Ctrl+ESCキーで実行
void CSekaijuDoc::OnEditTerminateSAL () {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	if (pSekaijuApp->m_bSALRunning && pSekaijuApp->m_pSAL != NULL) {
		SAL_Terminate pfnSAL_Terminate = (SAL_Terminate)pSekaijuApp->m_pfnSAL_Terminate;
		int nResult = pfnSAL_Terminate (pSekaijuApp->m_pSAL, 1);
	}
}

// 『編集(E)』-『SALの強制終了』非表示, Ctrl+ESCキーで実行
void CSekaijuDoc::OnUpdateEditTerminateSALUI (CCmdUI* pCmdUI) {
	;
}
...
void CSekaijuDoc::OnPopupTrackOutputOn () {
	...
	pSekaijuApp->m_theCriticalSection.Lock ();
	pSekaijuApp->ResetTempMIDIStatusArray ();
	this->TimeMIDIStatus (lCurTime, pSekaijuApp->m_lKeyShift, pSekaijuApp->m_pTempMIDIStatus);
	long lFlags;
...
void CSekaijuDoc::OnPopupInsertTempo () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		dTempoBPM = _tstof (theDlg.m_strTempoBPM);
		dTempoBPM = CLIP (4.00, dTempoBPM, 60000.00);
		lTempo = (long)(60000000 / dTempoBPM);
		BeginWaitCursor ();
		m_theCriticalSection.Lock ();
		...
		long lMillisec = MIDIData_TimeToMillisec (m_pMIDIData, lCurTime);
		MIDIClock_SetMillisec (m_pMIDIClock, lMillisec);
		MIDIData_FindTempo (m_pMIDIData, lTime, &lValue);
		MIDIClock_SetTempo (m_pMIDIClock, lValue);
		if (pSekaijuApp->m_bPlaying) {
			MIDIClock_Start (m_pMIDIClock);
		}
...
void CSekaijuDoc::OnPopupInsertMarker () {
	...
	LongTimeToStringTime (m_pMIDIData, lTime, &strTime);
	theDlg.m_strTime = strTime;
	TCHAR szText1[MIDIEVENT_MAXLEN];
	TCHAR szText2[MIDIEVENT_MAXLEN];
...
void CSekaijuDoc::OnPopupModifyTempo () {
	...
	if (theDlg.DoModal () == IDOK) {
		// タイム文字列をlong型に変換
		long lNewTime = lTime;
		long lErrorID = StringTimeToLongTime (m_pMIDIData, theDlg.m_strTime, &lNewTime);
		if (lErrorID != 0) {
			CString strMsg;
			VERIFY (strMsg.LoadString (lErrorID));
			AfxMessageBox (strMsg, MB_ICONEXCLAMATION);
		...
		if (lNewTime != lTime) {
			MIDIEvent_SetTime (pNewEvent, lNewTime);
		}
...
void CSekaijuDoc::OnPopupModifyTimeSignature () {
	...
	if (theDlg.DoModal () == IDOK) {
		...
		MIDIEvent_SetTimeSignature (pNewEvent, lnn, ldd, lcc, lbb); //20171018Fixed
		pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pNewEvent);
...
// 『ポップアップ』-『音色の選択...』
void CSekaijuDoc::OnPopupSelectVoice () {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bInplaceEditing ||
		pSekaijuApp->m_bInplaceListing ||
	...
	if (!MIDIEvent_IsProgramChange (m_pTempEvent)) {
		return;
	}

	long lTrackOutputPort = MIDITrack_GetOutputPort (m_pTempTrack);
	long lTrackOutputChannel = MIDITrack_GetOutputChannel (m_pTempTrack);
	long lTrackViewMode = MIDITrack_GetViewMode (m_pTempTrack);
	...
	if (pMIDIInstDef == NULL) {
		return;
	}

	// CC#0,CC#32Eventを探索。
	MIDIEvent* pPCEvent = m_pTempEvent;
	MIDIEvent* pCC0Event = MIDIEvent_GetPrevEvent (pPCEvent);
	MIDIEvent* pCC32Event = MIDIEvent_GetPrevEvent (pPCEvent);
	// CC#32あった場合
	if (pCC32Event != NULL && MIDIEvent_IsControlChange (pCC32Event) && 
		MIDIEvent_GetNumber (pCC32Event) == 32) {
		// CC#0を探す
		pCC0Event = MIDIEvent_GetPrevEvent (pCC32Event);
		if (pCC0Event != NULL && MIDIEvent_IsControlChange (pCC0Event) && 
			MIDIEvent_GetNumber (pCC0Event) == 0) {
		}
		else {
			pCC0Event = NULL;
		}
	...
	// CC#0あった場合
	else if (pCC0Event != NULL && MIDIEvent_IsControlChange (pCC0Event) && 
		MIDIEvent_GetNumber (pCC0Event) == 0) {
		// CC#32を探す
		pCC32Event = MIDIEvent_GetPrevEvent (pCC0Event);
		if (pCC32Event != NULL && MIDIEvent_IsControlChange (pCC32Event) && 
			MIDIEvent_GetNumber (pCC32Event) == 32) {
		}
		else {
	...
	// 音色の選択ダイアログ.DoModal
	CSelectVoiceDlg theDlg;
	theDlg.m_pMIDIInstrumentDefinition = (void*)pMIDIInstDef;
	theDlg.m_pCC0Event = pCC0Event;
	theDlg.m_pCC32Event = pCC32Event;
	theDlg.m_pPCEvent = pPCEvent;
	theDlg.m_strSearchString = pSekaijuApp->m_theSelectVoiceDlgStatus.m_strSearchString;
	theDlg.m_bCaseSensitive = pSekaijuApp->m_theSelectVoiceDlgStatus.m_bCaseSensitive;
	theDlg.m_bExactMatch = pSekaijuApp->m_theSelectVoiceDlgStatus.m_bExactMatch;
	theDlg.m_nListIndex = pSekaijuApp->m_theSelectVoiceDlgStatus.m_nListIndex;
	int nRet = theDlg.DoModal ();
	pSekaijuApp->m_theSelectVoiceDlgStatus.m_strSearchString = theDlg.m_strSearchString;
	pSekaijuApp->m_theSelectVoiceDlgStatus.m_bCaseSensitive = theDlg.m_bCaseSensitive;
	pSekaijuApp->m_theSelectVoiceDlgStatus.m_bExactMatch = theDlg.m_bExactMatch;
	pSekaijuApp->m_theSelectVoiceDlgStatus.m_nListIndex = theDlg.m_nListIndex;

	UpdateAllViews (NULL, SEKAIJUDOC_MIDIEVENTCHANGED);
	::Sleep (0);
}

// 音色の選択ダイアログで音色が変更されたとき又は適用ボタンが押されたとき
void CSekaijuDoc::ApplySelectVoiceDlg (CDialog* pDlg) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSelectVoiceDlg* pSelectVoiceDlg = (CSelectVoiceDlg*)pDlg;
	...
	MIDIEvent* pCC0Event = (MIDIEvent*)(pSelectVoiceDlg->m_pCC0Event);
	MIDIEvent* pCC32Event = (MIDIEvent*)(pSelectVoiceDlg->m_pCC32Event);
	MIDIEvent* pPCEvent = (MIDIEvent*)(pSelectVoiceDlg->m_pPCEvent);
	MIDIEvent* pCloneCC0Event = NULL;
	MIDIEvent* pCloneCC32Event = NULL;
	MIDIEvent* pClonePCEvent = NULL;
	long lChannel = MIDIEvent_GetChannel (pPCEvent);
	lChannel = CLIP (0, lChannel, 15);

	// CC#32なかった場合、CC#32イベントを新規作成してトラックに追加
	if (pCC32Event == NULL) {
		VERIFY (pCC32Event = MIDIEvent_CreateControlChange (pPCEvent->m_lTime, lChannel, 32, 0));
		VERIFY (MIDITrack_InsertEventBefore (m_pTempTrack, pCC32Event, pPCEvent));
		VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCC32Event));
	}
	// CC#0なかった場合、CC#0イベントを新規作成してトラックに追加
	if (pCC0Event == NULL) {
		VERIFY (pCC0Event = MIDIEvent_CreateControlChange (pPCEvent->m_lTime, lChannel, 0, 0));
		VERIFY (MIDITrack_InsertEventBefore (m_pTempTrack, pCC0Event, pCC32Event));
		VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCC0Event));
	}

	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pCC0Event));
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pCC32Event));
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pPCEvent));
	VERIFY (pCloneCC0Event = this->ReplaceMIDIEvent (pCC0Event));
	VERIFY (pCloneCC32Event = this->ReplaceMIDIEvent (pCC32Event));
	VERIFY (pClonePCEvent = this->ReplaceMIDIEvent (pPCEvent));
	if (0 <= pSelectVoiceDlg->m_nCC0 && pSelectVoiceDlg->m_nCC0 <= 127) {
		MIDIEvent_SetValue (pCloneCC0Event, CLIP (0, pSelectVoiceDlg->m_nCC0, 127));
	}
	if (0 <= pSelectVoiceDlg->m_nCC32 && pSelectVoiceDlg->m_nCC32 <= 127) {
		MIDIEvent_SetValue (pCloneCC32Event, CLIP (0, pSelectVoiceDlg->m_nCC32, 127));
	}
	if (0 <= pSelectVoiceDlg->m_nPC && pSelectVoiceDlg->m_nPC <= 127) {
		MIDIEvent_SetValue (pClonePCEvent, CLIP (0, pSelectVoiceDlg->m_nPC, 127));
	}
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneCC0Event));
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneCC32Event));
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pClonePCEvent));
	m_theCriticalSection.Unlock ();

	pSelectVoiceDlg->m_pCC0Event = pCloneCC0Event;
	pSelectVoiceDlg->m_pCC32Event = pCloneCC32Event;
	pSelectVoiceDlg->m_pPCEvent = pClonePCEvent;

	// MIDIメッセージ送信
	if (pSekaijuApp->m_theGeneralOption.m_bChangeUpdate) {
		m_theCriticalSection.Lock ();
		pSekaijuApp->m_theCriticalSection.Lock ();
		long lPort = MIDITrack_GetOutputPort (m_pTempTrack);
		long lChannel = MIDITrack_GetOutputChannel (m_pTempTrack);
		if (lChannel < 0) {
			lChannel = MIDIEvent_GetChannel (m_pTempEvent);
		}
		long lTime = CLIP (0, this->m_lNewTime - 1, 0x7FFFFFFF);
		pSekaijuApp->SendAllNoteOffEx (lPort, lChannel);
		pSekaijuApp->ResetTempMIDIStatusArray ();
		this->TimeMIDIStatus 
			(lTime, pSekaijuApp->m_lKeyShift, pSekaijuApp->m_pTempMIDIStatus);
		long lFlags = pSekaijuApp->m_bPlaying ? SDS_ALL : SDS_ALL & ~SDS_NOTE;
		pSekaijuApp->SendDifferentStatus (lFlags);
		m_theCriticalSection.Unlock ();
		pSekaijuApp->m_theCriticalSection.Unlock ();
...
// 『ポップアップ』-『音色の選択...』
void CSekaijuDoc::OnUpdatePopupSelectVoiceUI (CCmdUI* pCmdUI) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bInplaceEditing ||
		pSekaijuApp->m_bInplaceListing ||
...
// 『ポップアップ』-『システムエクスクルーシヴデータの読み込み』
void CSekaijuDoc::OnPopupLoadSysExData () {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	// リアルタイム入力中は何もしない。
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bInplaceEditing ||
	...
	if (!MIDIEvent_IsSysExEvent (m_pTempEvent)) {
		return;
	}

	CString strFileName;
	CSekaijuFileDlg theFileDlg (TRUE);

	CString strTitle;
	VERIFY (strTitle.LoadString (AFX_IDS_APP_TITLE));
	CString strInitialDir;
	strInitialDir = pSekaijuApp->m_strExeFilePath + _T("syx\\");
	TCHAR szOldCurrentDirectory[1024];
	memset (szOldCurrentDirectory, 0, sizeof (szOldCurrentDirectory));
	...
	CString strFilter;
	CString strDefault;
	CString strFilterSYX[3];
	VERIFY (strFilterSYX[0].LoadString (IDS_SYSTEM_EXCLUSIVE_DATA_FILES_AD_SYX));
	VERIFY (strFilterSYX[1].LoadString (IDS_AD_SYX));
	VERIFY (strFilterSYX[2].LoadString (IDS_D_SYX));

	// 拡張子フィルター追加した部分
	strFilter += strFilterSYX[0]; // システムエクスクルーシヴデータファイル(*.syx)
	strFilter += (TCHAR)_T('\0');
	strFilter += strFilterSYX[1]; // *.syx
	strFilter += (TCHAR)_T('\0');
	strFilter += (TCHAR)_T('\0');
	...
	// デフォルトの拡張子設定
	theFileDlg.m_strDefExt = strFilterSYX[2];
	theFileDlg.m_ofn.lpstrDefExt = theFileDlg.m_strDefExt;
	theFileDlg.m_ofn.lpstrInitialDir = pSekaijuApp->m_strExeFilePath;
	// フィルターコンボボックスのデフォルト選択
	if ((strFileName.Right (4)).CompareNoCase (strFilterSYX[2]) == 0) { // .syx
		theFileDlg.m_ofn.nFilterIndex = 1;
	}
	theFileDlg.m_ofn.Flags |= OFN_FILEMUSTEXIST;
	...
	if (theFileDlg.m_ofn.nFilterIndex == 1 &&
		(strFileName.Right (4)).CompareNoCase (strFilterSYX[2]) != 0) { // .csv
		strFileName += strFilterSYX[2];
	}
	...
	// ファイルオープン
	CFile theFile;
	BOOL bRet = theFile.Open (strFileName, CFile::modeRead);
	if (bRet == FALSE) {
		CString strMsg;
		CString strFormat;
	...
	// 新しい履歴の用意
	VERIFY (strHistoryName.LoadString (IDS_MODIFY_SYSEX_DATA));
	VERIFY (this->AddHistoryUnit (strHistoryName));
	VERIFY (pCurHistoryUnit = this->GetCurHistoryUnit ());

	// SYSXの読み込み
	ULONGLONG ullLen = theFile.GetLength ();
	long lLen = (long)CLIP (0, ullLen, 65535);
	BYTE* pData = NULL;
	pData = (BYTE*)malloc (lLen);
	if (pData == NULL) {
		theFile.Close ();
	}
	theFile.Read (pData, lLen);

	m_theCriticalSection.Lock ();
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, m_pTempEvent));
	VERIFY (pCloneEvent = this->ReplaceMIDIEvent (m_pTempEvent));
	MIDIEvent_SetData (pCloneEvent, pData, lLen);
	VERIFY (pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTEVENT, pCloneEvent));
	m_theCriticalSection.Unlock ();
	free (pData);
...
// 『ポップアップ』-『システムエクスクルーシヴデータの読み込み』
void CSekaijuDoc::OnUpdatePopupLoadSysExDataUI (CCmdUI* pCmdUI) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	// リアルタイム入力中は何もしない。
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bInplaceEditing ||
...
// 『ポップアップ』-『システムエクスクルーシヴデータの保存』
void CSekaijuDoc::OnPopupSaveSysExData () {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	// リアルタイム入力中は何もしない。
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bInplaceEditing ||
	...
	// SYSXの書き込み
	m_theCriticalSection.Lock ();
	theFile.Write (m_pTempEvent->m_pData, m_pTempEvent->m_lLen);
	m_theCriticalSection.Unlock ();
...
// 『ポップアップ』-『システムエクスクルーシヴデータの保存』
void CSekaijuDoc::OnUpdatePopupSaveSysExDataUI (CCmdUI* pCmdUI) {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	// リアルタイム入力中は何もしない。
	if (pSekaijuApp->m_bRecording ||
		pSekaijuApp->m_bInplaceEditing ||
...
// スレッド間通信用ダミーコマンド
void CSekaijuDoc::OnCommandStopPlaying () {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	pSekaijuApp->m_theCriticalSection.Lock ();
	this->m_theCriticalSection.Lock ();
	pSekaijuApp->StopPlaying (this);
	this->m_theCriticalSection.Unlock ();
	pSekaijuApp->m_theCriticalSection.Unlock ();
}

// スレッド間通信用ダミーコマンド
void CSekaijuDoc::OnCommandStopRecording () {
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	pSekaijuApp->m_theCriticalSection.Lock ();
	this->m_theCriticalSection.Lock ();
	pSekaijuApp->StopRecording (this);
