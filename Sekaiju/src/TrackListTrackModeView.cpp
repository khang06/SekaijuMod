...
// 指定したセルのMIDIEventを取得する(プログラムチェンジ・コントロールチェンジのみ)
void* CTrackListTrackModeView::GetCellEvent (long lRow, long lColumn) {
	...
	case TRACKLISTFRAME_CONTROLCHANGE:	
		forEachEvent (pMIDITrack, pMIDIEvent) {
			if (MIDIEvent_IsControlChange (pMIDIEvent) && 
				MIDIEvent_GetNumber (pMIDIEvent) == lNumber) {
				return pMIDIEvent;
			}
		}
		return NULL;
...
BOOL CTrackListTrackModeView::EndValueUpDown () {
	...
	long lColumnContent = pTrackListFrame->GetColumnContent (m_lCurColumn);
	long lNumber = lColumnContent >> 16;
	MIDITrack* pTempTrack = pSekaijuDoc->GetTrack (m_lCurRow);
	MIDIEvent* pTempEvent = (MIDIEvent*)GetCellEvent (m_lCurRow, m_lCurColumn);
	// 現在の履歴の用意
	CHistoryUnit* pCurHistoryUnit = NULL;
	VERIFY (pCurHistoryUnit = pSekaijuDoc->GetCurHistoryUnit ());
	// 項目別に
...
BOOL CTrackListTrackModeView::ToggleValueOfCurCell () {
	...
	case TRACKLISTFRAME_OUTPUTON:
		...
		if (lValue == 0) {
			...
			pSekaijuApp->ResetTempMIDIStatusArray ();
			pSekaijuDoc->TimeMIDIStatus (lCurTime, pSekaijuApp->m_lKeyShift, pSekaijuApp->m_pTempMIDIStatus);
			long lFlags;
			// 再生中ならばノートのみ復元
...
BOOL CTrackListTrackModeView::SetCellString (long lRow, long lColumn, CString strText) {
	...
	case TRACKLISTFRAME_OUTPUTPORT:
		...
		pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_INSERTTRACK, pCloneTrack); // 20180331追加
		m_bSettingCellString = 0;
		return TRUE;
	// 出力チャンネル
	case TRACKLISTFRAME_OUTPUTCHANNEL:
...
BOOL CTrackListTrackModeView::MoveTextBox (long lRow, long lColumn) {
	...
	m_theTextBox.MoveWindow 
		(rcNewCell.left, rcNewCell.top + pTrackListFrame->GetRowZoom () / 2 - lHeight/ 2, 
		rcNewCell.Width () - lButtonWidth, lHeight);
...
// マウス中ボタン押された時  
void CTrackListTrackModeView::OnMButtonDown (UINT nFlags, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*) GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// 録音中は何もしない
...
void CTrackListTrackModeView::OnRButtonDown (UINT nFlags, CPoint point) {
	...
	if ((lColumnContent & 0xFFFF) == TRACKLISTFRAME_INPUTPORT ||
		...
		// ボタンは押されていない場合
			...
			CMenu theMenu;
			// プログラムナンバーの場合(音色の選択...項目付き)
			if ((lColumnContent & 0xFFFF) == TRACKLISTFRAME_PROGRAMCHANGE) {
				if (pSekaijuDoc->m_pTempTrack) {
					long lChannel = MIDITrack_GetOutputChannel (pSekaijuDoc->m_pTempTrack);
					pSekaijuDoc->m_pTempEvent = 
						MIDITrack_GetFirstKindEvent 
						(pSekaijuDoc->m_pTempTrack, MIDIEVENT_PROGRAMCHANGE | lChannel);
				}
				VERIFY (theMenu.LoadMenu (IDR_POPUPMENU05));
			}
			// その他
			else {
...
// マウス中ボタン離されたとき
void CTrackListTrackModeView::OnMButtonUp (UINT nFlags, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*) GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
}
...
void CTrackListTrackModeView::OnTimer (UINT nIDEvent) {	
	...
	// ユニバーサルスクロール処理
		...
		long lX = pTrackListFrame->GetColumnScrollPos ();
		long lY = pTrackListFrame->GetRowScrollPos ();
		long lDeltaX = m_ptMouseMoveM.x - m_ptMouseDownM.x;
		long lDeltaY = m_ptMouseMoveM.y - m_ptMouseDownM.y;
		pTrackListFrame->SetColumnScrollPos (lX + lDeltaX / 4);
		pTrackListFrame->SetRowScrollPos (lY + lDeltaY / 4);
	}
