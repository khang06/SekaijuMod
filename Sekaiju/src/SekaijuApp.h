...
#define WM_COMMANDSTOPPLAYING (WM_USER + 16) // 演奏録音スレッド→メインスレッド通信用
#define WM_COMMANDSTOPRECORDING (WM_USER + 17) // 演奏録音スレッド→メインスレッド通信用
#define WM_COMMANDUPDATEALLVIEWS (WM_USER + 30)


// 共有メモリ関係
#define SHMSIZE 1024                    // プロセス間通信用共有メモリのサイズ
...
// ストロークダイアログの状態
typedef struct {
	int m_nDeltaTime;                   // デルタタイム(1～)[tick]
	int m_nMode;                        // モード(0=Up,1=Down,2=Up-Down,3=DwonUp)
} EditStrokeDlgStatus;
...
// 言語ダイアログの状態
typedef struct {
	CString m_strUserInterface;         // ユーザーインターフェイス言語
...
// フォントオプションページの状態
typedef struct {
	CString m_strFontName[4];           // フォント名[4](""のときはIDS_DEFAULTFONTNAMEとする)
	long m_lFontSize[4];                // フォントサイズ[4](スクリーン用はpixel,印刷用は0.1mm単位)
} FontOption;
...
typedef struct {
	...
	long m_lMIDISyncMode;               // MIDI同期モード
	long m_lOption;                     // オプション
} CurrentPage;

// 音色の検索と選択ダイアログの設定
typedef struct {
	CString m_strSearchString;          // 検索文字列
	BOOL m_bCaseSensitive;              // 大文字と小文字を区別する
	BOOL m_bExactMatch;                 // 完全一致
	int m_nListIndex;                   // リストインデックス
...
public:
	...
	MIDIInstrument* m_pMIDIInstrument[MAXMIDIINSTRUMENTNUM];
...
public:
	HMODULE m_hSALDLL;
	FARPROC m_pfnSAL_Create;
	FARPROC m_pfnSAL_Delete;
	FARPROC m_pfnSAL_SelectFile;
	FARPROC m_pfnSAL_LoadFile;
	FARPROC m_pfnSAL_Run;
	FARPROC m_pfnSAL_Terminate;
	FARPROC m_pfnSAL_AddToForEachEventArray;
	void* m_pSAL;
...
public:
	...
	// MIDIスルーの設定
	BOOL m_bMIDIThru[MAXMIDIINDEVICENUM];
	...
	// EditTempoダイアログ状態
	EditTempoDlgStatus m_theEditTempoDlgStatus;
	...
	// EditStrokeダイアログ状態
	EditStrokeDlgStatus m_theEditStrokeDlgStatus;
	...
	// 音色の検索と選択ダイアログ状態
	SelectVoiceDlgStatus m_theSelectVoiceDlgStatus;
	...
	// 言語ダイアログ状態
	LanguageDlgStatus m_theLanguageDlgStatus;
...
public:
	...
	virtual void SendAllNoteOffEx (long lPort, long lChannel);
	...
	virtual BOOL IsDocumentExist (void* pDoc);
...
public:
	// SALスレッド関連
	static UINT SALThread (LPVOID pInfo);
	CWinThread* m_pSALThread;
	BOOL m_bSALRunning;
...
protected:
	...
	afx_msg void OnControlKeyShift (UINT nID);
	afx_msg void OnUpdateControlKeyShiftUI (CCmdUI* pCmdUI);
