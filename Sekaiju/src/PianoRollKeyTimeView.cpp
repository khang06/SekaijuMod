...
void CPianoRollKeyTimeView::OnTimer (UINT nIDEvent) {
	...
	// ユニバーサルスクロール処理
		...
		long lY = pPianoRollFrame->GetKeyScrollPos ();
		...
		pPianoRollFrame->SetKeyScrollPos (lY + lDeltaY / 4);
	}
...
// マウス中ボタン押された時  
void CPianoRollKeyTimeView::OnMButtonDown (UINT nFlags, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// 録音中は何もしない
...
// マウス中ボタン離されたとき
void CPianoRollKeyTimeView::OnMButtonUp (UINT nFlags, CPoint point) {
	CPianoRollFrame* pPianoRollFrame = (CPianoRollFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
}
