...
void CTrackListTrackTimeView::OnTimer (UINT nIDEvent) {
	...
	// ユニバーサルスクロール処理
		...
		long lX = pTrackListFrame->GetTimeScrollPos ();
		...
		pTrackListFrame->SetTimeScrollPos (lX + lDeltaX / 4);
...
// マウス中ボタン押された時  
void CTrackListTrackTimeView::OnMButtonDown (UINT nFlags, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// 録音中は何もしない
...
// マウス中ボタン離されたとき
void CTrackListTrackTimeView::OnMButtonUp (UINT nFlags, CPoint point) {
	CTrackListFrame* pTrackListFrame = (CTrackListFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
}
