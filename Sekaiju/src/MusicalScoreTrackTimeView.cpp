...
void CMusicalScoreTrackTimeView::OnDraw (CDC* pDC) {
	...
	pOldFont = pDC->SelectObject (&(pSekaijuApp->m_theFont[1]));
	...
	pOldFont = pDC->SelectObject (&(pSekaijuApp->m_theFont[0]));
	i = 0;
	forEachTrack (pMIDIData, pMIDITrack) {
		if (pMusicalScoreFrame->IsTrackVisible (i)) {
			if (lVisibleTopTrack <= i && i <= lVisibleBottomTrack) {
...
void CMusicalScoreTrackTimeView::OnTimer (UINT nIDEvent) {
	...
	// ユニバーサルスクロール処理
		...
		long lX = pMusicalScoreFrame->GetTimeScrollPos ();
		long lY = pMusicalScoreFrame->GetTrackScrollPos ();
		long lDeltaX = m_ptMouseMoveM.x - m_ptMouseDownM.x;
		long lDeltaY = m_ptMouseMoveM.y - m_ptMouseDownM.y;
		pMusicalScoreFrame->SetTimeScrollPos (lX + lDeltaX / 4);
		pMusicalScoreFrame->SetTrackScrollPos (lY + lDeltaY / 4);
	}
}
...
void CMusicalScoreTrackTimeView::OnLButtonDown (UINT nFlags, CPoint point) {
	...
	case ID_MUSICALSCORE_PEN:
				...
				for (j = 0; j < lNumNoteInfo; j++) {
					...
					if (rcNote.top <= point.y && point.y < rcNote.bottom) {
						if (rcNote.left <= point. x && point.x < rcNote.right) {
							...
							if (pMusicalScoreFrame->m_bTie) { // タイの入力開始
								m_lTempMode = 0x0109;
							}
							else { // 既存音符の移動開始
								m_lTempMode = 0x0103;
		...
		if (m_pTempEvent == NULL) {
			...
			m_lTempChannel = MIDITrack_GetOutputChannel (m_pTempTrack); // 20190223追加
...
// マウス中ボタン押された時  
void CMusicalScoreTrackTimeView::OnMButtonDown (UINT nFlags, CPoint point) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	// 録音中は何もしない
...
// マウス中ボタン離されたとき
void CMusicalScoreTrackTimeView::OnMButtonUp (UINT nFlags, CPoint point) {
	CMusicalScoreFrame* pMusicalScoreFrame = (CMusicalScoreFrame*)GetParent ();
	CSekaijuApp* pSekaijuApp = (CSekaijuApp*)AfxGetApp ();
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	...
	if (GetCapture () == this) {
		...
		case ID_MUSICALSCORE_PEN:
			...
			// タイ入力中
				...
				// 時刻が変わった場合のみかつまだタイが入力されてない場合のみ
				if ((lOldTime / m_lTempSnap) * m_lTempSnap != 
					(lNewTime / m_lTempSnap) * m_lTempSnap &&
					!m_bTempTieInserted) {
					MIDIEvent* pEvent = NULL;
					if (lOldTime < lNewTime) {
						pEvent = m_pTempEvent->m_pNextEvent;
						while (pEvent != NULL) {
							if (MIDIEvent_IsNoteOn (pEvent) &&
								MIDIEvent_GetChannel (pEvent) == MIDIEvent_GetChannel (m_pTempEvent) &&
								MIDIEvent_GetKey (pEvent) == MIDIEvent_GetKey (m_pTempEvent)) {
								break;
							}
						...
						pEvent = m_pTempEvent->m_pPrevEvent;
						while (pEvent != NULL) {
							if (MIDIEvent_IsNoteOn (pEvent) &&
								MIDIEvent_GetChannel (pEvent) == MIDIEvent_GetChannel (m_pTempEvent) &&
								MIDIEvent_GetKey (pEvent) == MIDIEvent_GetKey (m_pTempEvent)) {
					...
					if (pEvent != NULL) {
						long lOnTime1 = MIDIEvent_GetTime (m_pTempEvent);
						long lOnTime2 = MIDIEvent_GetTime (pEvent);
						long lOnTime = MIN (lOnTime1, lOnTime2);
						long lOffTime1 = MIDIEvent_GetTime (m_pTempEvent) + MIDIEvent_GetDuration (m_pTempEvent);
						long lOffTime2 = MIDIEvent_GetTime (pEvent) + MIDIEvent_GetDuration (pEvent);
						long lOffTime = MAX (lOffTime1, lOffTime2);
						long lDuration = CLIP (1, lOffTime - lOnTime, 65535);
						MIDIEvent_SetTime (m_pTempEvent, lOnTime);
						MIDIEvent_SetDuration (m_pTempEvent, lDuration);
						pCurHistoryUnit->AddHistoryRecord (HISTORYRECORD_REMOVEEVENT, pEvent);
						MIDITrack_RemoveEvent (m_pTempTrack, pEvent);
						m_bTempTieInserted = TRUE;
						//pSekaijuDoc->SetModifiedFlag (TRUE);
						pMusicalScoreFrame->UpdateNoteInfoArray (m_lTempTrackIndex, m_pTempTrack);
						pMusicalScoreFrame->UpdateNoteGroupInfoArray (m_lTempTrackIndex, m_pTempTrack);
						Invalidate ();
