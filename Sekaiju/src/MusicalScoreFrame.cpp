...
//	ON_WM_CHILDACTIVATE ()	
	...
	ON_COMMAND (ID_MUSICALSCORE_TIE, OnMusicalScoreTie)
	ON_UPDATE_COMMAND_UI (ID_MUSICALSCORE_TIE, OnUpdateMusicalScoreTieUI)
	...
	ON_BN_CLICKED (IDC_FILLCHECKBOX, OnFillCheckClicked)
	ON_BN_CLICKED (IDC_TRIMCHECKBOX, OnTrimCheckClicked)
...
BOOL CMusicalScoreFrame::UpdateNoteInfoArray (long lTrackIndex, MIDITrack* pMIDITrack) {
	...
	// フィル
	if (m_wndFillCheckBox.GetCheck () > 0) {
		long lTimeResolution = MIDIData_GetTimeResolution (pMIDIData);
		long lNextNoteOn = 0x00000000;
		forEachEvent(pMIDITrack, pMIDIEvent) {
			if (MIDIEvent_IsNoteOn (pMIDIEvent) && MIDIEvent_IsNote (pMIDIEvent)) {
				MIDIEvent* pMIDIEvent2 = pMIDIEvent;
				forEachEvent (pMIDITrack, pMIDIEvent2) {
					if (MIDIEvent_IsNoteOn (pMIDIEvent2) && MIDIEvent_IsNote (pMIDIEvent2)) {
						if (pMIDIEvent2->m_lUser1 > pMIDIEvent->m_lUser1) {
							lNextNoteOn = pMIDIEvent2->m_lUser1;
							break;
						}
					}
				}
				...
				// 8分音符未満の隙間なら
				if (lNextNoteOn > pMIDIEvent->m_lUser2 &&
					lNextNoteOn - pMIDIEvent->m_lUser2 < lTimeResolution / 2) {
					pMIDIEvent->m_lUser2 = MAX (pMIDIEvent->m_lUser2, MIN (lNextNoteOn, lNextNoteOn));
				}
			}
		}
	}
	...
	// トリム
	if (m_wndTrimCheckBox.GetCheck () > 0) {
		long lTimeResolution = MIDIData_GetTimeResolution (pMIDIData);
		long lNextNoteOn = 0x00000000;
		forEachEvent(pMIDITrack, pMIDIEvent) {
			if (MIDIEvent_IsNoteOn (pMIDIEvent) && MIDIEvent_IsNote (pMIDIEvent)) {
				...
				// 8分音符以下の重複ならば
				if (lNextNoteOn < pMIDIEvent->m_lUser2  &&
					pMIDIEvent->m_lUser2 - lNextNoteOn <= lTimeResolution / 2) {
					pMIDIEvent->m_lUser2 = lNextNoteOn;
				}
			}
		}
	}
			...
			if (pNoteInfo->m_pNoteGroupInfo == NULL) {
				...
				if (j + 1 < lNumNoteInfo) {
					...
					// 次の音符の所属拍が現在の音符の所属拍と異なる場合
						...
						// 3連8分音符以下が検出されている場合は拍境界で必ず切断
						if (bBeatStopReady == TRUE) { 
							bGroupContinueFlag = FALSE; // 現在の音符グループは終了
							bBeatStopReady = FALSE;
						}
						// 4/4の3拍目と2/2の2拍目では必ず切断
						else if (pNoteInfo->m_lnn == 4 && pNoteInfo->m_ldd == 2 && pNextNoteInfo->m_lNoteOnBeat == 2 ||
								 pNoteInfo->m_lnn == 2 && pNoteInfo->m_ldd == 1 && pNextNoteInfo->m_lNoteOnBeat == 1) {
...
BOOL CMusicalScoreFrame::AddNoteInfo (MusicalScoreTrackInfo* pTrackInfo, MusicalScoreNoteInfo* pNoteInfo) {
	...
	long j;
	long lcc, lbb;
	// pNoteInfoのlnnとlddを取得
	CSekaijuDoc* pSekaijuDoc = (CSekaijuDoc*)GetDocument ();
	MIDIData* pMIDIData = pSekaijuDoc->m_pMIDIData;
	MIDIData_FindTimeSignature (pMIDIData, pNoteInfo->m_lNoteOnTime, &(pNoteInfo->m_lnn), &(pNoteInfo->m_ldd), &lcc, &lbb);
...
int CMusicalScoreFrame::OnCreate (LPCREATESTRUCT lpCreateStruct) {
	...
	// 表示精度コンボの作成
	m_wndToolBar1.SetButtonInfo (25, IDC_RESOLUTIONCOMBO, TBBS_SEPARATOR, 100);
	m_wndToolBar1.GetItemRect (25, &rcTemp);
	rcTemp.top = 2;
	rcTemp.bottom = 120;
	if (!m_wndResolutionCombo.CreateEx (
		...
		TRACE0 ("Failed to create Resolution Combo Box\n");
		return -1;
	}
	...
	// フィルチェックボックス作成
	m_wndToolBar1.SetButtonInfo (27, IDC_FILLCHECKBOX, TBBS_SEPARATOR, 48);
	//VERIFY (m_wndToolBar1.GetToolBarCtrl ().HideButton (IDC_FILLCHECKBOX, TRUE)); //20190305
	/*TBBUTTONINFO tbinfo;
    tbinfo.cbSize = sizeof(tbinfo);
    tbinfo.dwMask = TBIF_STATE | TBIF_STYLE | TBIF_SIZE;
    tbinfo.fsState = TBSTATE_HIDDEN;
	tbinfo.fsStyle = TBSTYLE_BUTTON;
	tbinfo.cx = 48;
	::SendMessage (m_wndToolBar1.GetSafeHwnd(), TB_SETBUTTONINFO, IDC_FILLCHECKBOX, (LPARAM)(&tbinfo));*/
	m_wndToolBar1.GetItemRect (27, &rcTemp);
	rcTemp.top = 2;
	rcTemp.bottom = 20;
	if (!m_wndFillCheckBox.CreateEx (
		0, _T("Button"), NULL,
		WS_VISIBLE | WS_TABSTOP | WS_CHILD | BS_AUTOCHECKBOX,
		rcTemp.left, rcTemp.top, rcTemp.Width (), rcTemp.Height (), 
		m_wndToolBar1.GetSafeHwnd (), (HMENU)IDC_FILLCHECKBOX)) {
		TRACE0 ("Failed to create Fill Check Box\n");
		return -1;
	}
	m_wndFillCheckBox.SetFont (CFont::FromHandle ((HFONT)::GetStockObject (DEFAULT_GUI_FONT)));
	m_wndFillCheckBox.SetWindowText (_T("Fill"));

	// トリムチェックボックスの作成
	m_wndToolBar1.SetButtonInfo (29, IDC_TRIMCHECKBOX, TBBS_SEPARATOR, 48);
	m_wndToolBar1.GetItemRect (29, &rcTemp);
	rcTemp.top = 2;
	rcTemp.bottom = 20;
	if (!m_wndTrimCheckBox.CreateEx (
		0, _T("BUTTON"), NULL,
		WS_VISIBLE | WS_TABSTOP | WS_CHILD | BS_AUTOCHECKBOX,
		rcTemp.left, rcTemp.top, rcTemp.Width (), rcTemp.Height (), 
		m_wndToolBar1.GetSafeHwnd (), (HMENU)IDC_TRIMCHECKBOX)) {
		TRACE0 ("Failed to create Trim Check Box");
		return -1;
	}
	m_wndTrimCheckBox.SetFont (CFont::FromHandle ((HFONT)::GetStockObject (DEFAULT_GUI_FONT)));
	m_wndTrimCheckBox.SetWindowText (_T("Trim"));
	...
	// フィル
	CString strFill;
	strFill.LoadString (IDS_FILL); //LoadString (pSekaijuApp->m_hResourceDLL, IDS_FILL);
	m_wndFillCheckBox.SetWindowTextW (strFill);

	// トリム
	CString strTrim;
	strTrim.LoadString (IDS_TRIM); //= LoadString (pSekaijuApp->m_hResourceDLL, IDS_TRIM);
	m_wndTrimCheckBox.SetWindowTextW (strTrim);
...
void CMusicalScoreFrame::OnKeyDown (UINT nChar, UINT nRepCnt, UINT nFlags) {
	...
	case 't' :
		if (GetCapture () == NULL) {
			this->PostMessage (WM_COMMAND, ID_MUSICALSCORE_TIE, 0);
		}
		break;
	// ↑
	case VK_UP:
...
void CMusicalScoreFrame::OnTimeZoomDown () {
	...
	long lNewTimeZoom = CLIP (1, m_lTimeZoom - 1, 32);
...
// 『ツール(&T)』-『タイ』
void CMusicalScoreFrame::OnMusicalScoreTie () {
	m_bTie = !m_bTie;
}

// 『ツール(&T)』-『タイ』
void CMusicalScoreFrame::OnUpdateMusicalScoreTieUI (CCmdUI* pCmdUI) {
	pCmdUI->Enable (TRUE);	
	pCmdUI->SetCheck (m_bTie);
}
...
// フィルチェックボックスが変化したとき
void CMusicalScoreFrame::OnFillCheckClicked () {
	// 全トラックの音符情報と音符グループ情報を更新する
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	pSekaijuDoc->m_theCriticalSection.Lock ();
	UpdateTrackInfoArray ();
...
// トリムチェックボックスが変化したとき
void CMusicalScoreFrame::OnTrimCheckClicked () {
	// 全トラックの音符情報と音符グループ情報を更新する
	CSekaijuDoc* pSekaijuDoc = GetDocument ();
	pSekaijuDoc->m_theCriticalSection.Lock ();
	UpdateTrackInfoArray ();
